...
class ProteinDiseaseBridgeModel(nn.Module):
    ...
    def embed_seq(self, seq_feat_dict):
        ...
        # seq_emb_raw = self.seq_encoder(
        #     seq_feat_dict['input_ids'],
        #     attention_mask=seq_feat_dict['attention_mask'],
        #     # token_type_ids=torch.zeros(seq_feat_dict['input_ids'].size(), dtype=torch.long, device=self.device),
...
def parse_args():
    ...
    parser.add_argument("-c", "--config", help="configuration file", default='./configs/biobridge_pred.yaml')
...
def main():
    ...
    df_dis = pd.read_csv(data_configs['disease_table_file']).drop_duplicates().reset_index(drop=True)
    phenotype_vocab = df_dis['mondo_name'].to_list()
    # pheno_desc_dict = dict(zip(df_dis['mondo_name'], df_dis['definition']))
    ...
    with open(config['disease_emb_cache'], "rb") as f:
        dis_raw = pickle.load(f)
    # bb_dis_emb = torch.tensor(bb_dis_raw["embedding"], dtype=torch.float32)
    # bb_dis_idx = torch.tensor(bb_dis_raw["node_index"])
    dis_emb_raw = np.array(dis_raw['embedding'])
    dis_idx = dis_raw["node_index"]
    df_dis['idx'] = df_dis['node_index'].apply(lambda x: dis_idx.index(x))
    disease2idx = dict(zip(df_dis['mondo_name'], df_dis['idx']))  # disease name --> index in pre-computed embedding (from BioBridge release)
