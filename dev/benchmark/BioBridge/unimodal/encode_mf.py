...
def main(
    ...
    # tokenized_data_dir = os.path.join(data_path, "./encoded_mf")

    # load data
    # if not os.path.exists(tokenized_data_dir):
    def tokenize_data(datapoint):
        seq = "Name: {}. Definition: {}".format(datapoint["node_name"], datapoint["description"])
        tokenized = tokenizer(
                seq,
                truncation=True,
                padding=False,
        ...
        return tokenized
    data = load_dataset("csv", data_files=os.path.join(data_path, "molecular.csv"))
    ...
    data = data.remove_columns(["node_id","node_index","node_name","node_type","description","node_source"])
    ...
    with open(os.path.join(output_path, "mf.pkl"), "wb") as f:
        pickle.dump(
            outputs,
            f,
        )
