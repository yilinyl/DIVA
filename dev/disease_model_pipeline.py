...
def train_epoch(model, optimizer, device, data_loader, diagnostic=None, w_l=0.5, opt_patho=True, 
    ...
    for batch_idx, batch_data in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        patho_logits, batch_weights, seq_pheno_emb, pos_emb_proj, neg_emb_proj, _ = model(variant_data, desc_feat_dict)  # seq_pheno_emb: (pheno_vars_in_batch, hidden_size)
...
def eval_epoch(model, device, data_loader, pheno_vocab_emb, w_l=0.5):
    model.eval()
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        train_labels, train_scores, train_vars, train_adj_weights, train_pheno_results = eval_epoch(model, device, train_loader, pheno_vocab_emb=all_pheno_embs, w_l=model_args['w_l'])
        ...
        val_labels, val_scores, val_vars, val_adj_weights, val_pheno_results = eval_epoch(model, device, validation_loader, pheno_vocab_emb=all_pheno_embs, w_l=model_args['w_l'])
        ...
        test_labels, test_scores, test_vars, test_adj_weights, test_pheno_results = eval_epoch(model, device, test_loader, pheno_vocab_emb=all_pheno_embs, w_l=model_args['w_l'])
