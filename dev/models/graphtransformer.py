...
class GraphTransformer(nn.Module):
    ...
    def forward(self, g, h_lap_pos_enc, alt_aa, var_idx, ref_aa_key='ref_aa', feat_key='feat'):
        ...
        # the following is draft for recommender system
        # aa_idx = []
        # aa_mask = torch.zeros(20, dtype=torch.bool).to(self.device)
        # ref_aa = g.ndata['ref_aa'].detach()
        # for i in range(20):
        #     matches = (ref_aa == i).nonzero()
        #     if matches.size(0) == 0:
        #         aa_idx.append(torch.tensor(-1))
        #     else:
        #         aa_mask[i] = True
        #         aa_idx.append(matches[0].detach().squeeze(0))
        #
        # aa_idx = torch.stack(aa_idx)
        # h_aa_uniq = h_aa[aa_idx]
        # # h_aa_uniq = h_aa[aa_idx]
        # scores = torch.matmul(hg, h_aa_uniq.transpose(1, 0))
